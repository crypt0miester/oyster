...
test('extendTransactionBuffer', async () => {
  ...
  // Assert
  const transactionBuffer = await realm.getTransactionBuffer(realm.proposalTransactionBufferPk);
  // Note: Actual buffer content verification depends on how extension works on-chain
  expect(transactionBuffer.buffer.length).toBeGreaterThan(3); // Original was [1,2,3]
});
