...
import { getRealms, GoverningTokenConfigAccountArgs, GoverningTokenType } from "../../src";
...
test("setRealmConfig", async () => {
	...
	// councilTokenConfig
	expect(realmConfig.account.councilTokenConfig.tokenType).toEqual(GoverningTokenType.Liquid);
	expect(realmConfig.account.councilTokenConfig.voterWeightAddin).toEqual(undefined);
	expect(realmConfig.account.councilTokenConfig.maxVoterWeightAddin).toEqual(undefined);
});
...
test("getRealmsForMultiplePrograms", async () => {
	...
	expect(realms.map((r) => r.pubkey.toBase58())).toContainEqual(realm.realmPk.toBase58());
});
