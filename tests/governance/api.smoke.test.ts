...
import { withSetRealmConfig } from "../../lib/governance/withSetRealmConfig";
...
test("setupRealm", async () => {
	...
	const instruction = createMintToInstruction(mintPk, ataPk, governancePk, 1, [], TOKEN_PROGRAM_ID);
	...
	if (cancelProposal) {
		...
		await withExecuteTransaction(instructions, programId, programVersion, governancePk, proposalPk, transactionPk, [
			instructionData,
		]);
	}
	...
	withWithdrawGoverningTokens(instructions, programId, programVersion, realmPk, ataPk, mintPk, tokenOwnerRecordPk);

	await withUpdateProgramMetadata(instructions, programId, programVersion, walletPk);
