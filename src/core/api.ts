...
export function isWalletNotConnectedError(error: any): error is WalletNotConnectedError {
...
export const pubkeyFilter = (offset: number, pubkey: PublicKey | undefined | null) =>
...
export async function getBorshProgramAccounts<TAccount extends ProgramAccountWithType>(
	...
	const accountTypeFixed = accountType ?? new accountFactory({}).accountType;
		...
		filters: [
			{
				memcmp: {
					offset: 0,
					bytes: bs58.encode([accountTypeFixed]),
				},
			},
			...filters.map((f) => ({
