...
import { getGovernanceSchemaForAccount, GovernanceAccountParser } from "./serialisation";
import {
	getAccountTypes,
...
import { getBorshProgramAccounts, type MemcmpFilter, pubkeyFilter } from "../core/api";
...
export async function getRealms(connection: Connection, programIds: PublicKey | PublicKey[]) {
...
async function _getRealms(connection: Connection, programIds: PublicKey[]) {
	const accountTypes = getAccountTypes(Realm as GovernanceAccountClass);
	...
	for (const accountType of accountTypes) {
		...
		rawProgramAccounts.push(...programAccountsJson?.data?.filter((x: any) => x.result).flatMap((x: any) => x.result));
	}

	const accounts: ProgramAccount<Realm>[] = [];
	...
	for (const rawAccount of rawProgramAccounts) {
		try {
			...
			const account: ProgramAccount<Realm> = {
				...
				account: deserializeBorsh(getGovernanceSchemaForAccount(accountType), Realm, data),
				owner: rawAccount.account.owner,
			};

			accounts.push(account);
		} catch (ex) {
			console.info(`Can't deserialize Realm @ ${rawAccount.pubkey}.`, getErrorMessage(ex));
		}
	}
	return accounts;
}
...
export async function tryGetRealmConfig(connection: Connection, programId: PublicKey, realmPk: PublicKey) {
	try {
		...
		return await getGovernanceAccount(connection, realmConfigPk, RealmConfigAccount);
	} catch {
		// RealmConfigAccount didn't exist in V1 and was optional in V2 and hence it doesn't have to exist
	}
}

export async function getRealmConfig(connection: Connection, realmConfigPk: PublicKey) {
	return getGovernanceAccount(connection, realmConfigPk, RealmConfigAccount);
}

// VoteRecords

export async function getVoteRecord(connection: Connection, voteRecordPk: PublicKey) {
 ...
 * @returns
 */
export async function getAllGovernances(connection: Connection, programId: PublicKey, realmPk: PublicKey) {
 ...
 * @returns
 */
export async function getProposalsByGovernance(connection: Connection, programId: PublicKey, governancePk: PublicKey) {
 ...
 * @returns
 */
export async function getAllProposals(connection: Connection, programId: PublicKey, realmPk: PublicKey) {
	return getAllGovernances(connection, programId, realmPk).then((gs) =>
		Promise.all(gs.map((g) => getProposalsByGovernance(connection, programId, g.pubkey))),
	);
}
...
export async function getProposalDepositsByDepositPayer(
	...
	return getGovernanceAccounts(connection, programId, ProposalDeposit, [pubkeyFilter(1 + 32, depositPayer)!]);
}
...
export async function tryGetGovernanceAccount<TAccount extends GovernanceAccount>(
	connection: Connection,
	accountPk: PublicKey,
