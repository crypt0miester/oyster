...
import { accountsForTransactionExecute } from "../tools";
...
(BinaryReader.prototype as any).readVoteType = function () {
	...
	return VoteType.MULTI_CHOICE(choiceType, minVoterOptions, maxVoterOptions, maxWinningOptions);
};
...
(BinaryReader.prototype as any).readVoteThreshold = function () {
	...
	if (type === VoteThresholdType.YesVotePercentage || type === VoteThresholdType.QuorumPercentage) {
...
(BinaryWriter.prototype as any).writeVoteThreshold = function (value: VoteThreshold) {
	const writer = this as unknown as BinaryWriter;
	writer.maybeResize();
	writer.buf.writeUInt8(value.type, writer.length);
	writer.length += 1;
	...
	if (value.type === VoteThresholdType.YesVotePercentage || value.type === VoteThresholdType.QuorumPercentage) {
...
export const serializeInstructionToBase64 = (instruction: TransactionInstruction) => {
	const data = createInstructionData(instruction);

	return Buffer.from(serialize(GOVERNANCE_INSTRUCTION_SCHEMA_V1, data)).toString("base64");
};
...
// Instruction schemas
export const GOVERNANCE_INSTRUCTION_SCHEMA_V1 = createGovernanceInstructionSchema(1);
export const GOVERNANCE_INSTRUCTION_SCHEMA_V2 = createGovernanceInstructionSchema(2);
export const GOVERNANCE_INSTRUCTION_SCHEMA_V3 = createGovernanceInstructionSchema(3);
...
function createGovernanceStructSchema(programVersion: number | undefined, accountVersion: number | undefined) {
...
function createGovernanceInstructionSchema(programVersion: number) {
	return new Map<Function, any>([
		...
		[
			CreateProposalArgs,
			{
				...
				fields: [
					...
					programVersion >= PROGRAM_VERSION_V3 ? ["proposalSeed", "pubkey"] : undefined,
				].filter(Boolean),
			},
		],
		[
		...
		[
			...
			{
				kind: "struct",
				fields: [["instruction", "u8"], programVersion === PROGRAM_VERSION_V1 ? ["yesNoVote", "u8"] : ["vote", "vote"]],
			},
		],
		[
			InsertTransactionArgs,
			{
				...
				fields: [
					["instruction", "u8"],
					programVersion >= PROGRAM_VERSION_V2 ? ["optionIndex", "u8"] : undefined,
					["index", "u16"],
					["holdUpTime", "u32"],
		...
		[
			Proposal,
			{
				...
				fields: [
					...
					...(accountVersion === ACCOUNT_VERSION_V1 ? [] : [["maxVotingTime", { kind: "option", type: "u32" }]]),

					["voteThreshold", { kind: "option", type: "VoteThreshold" }],

					...(accountVersion === ACCOUNT_VERSION_V1 ? [] : [["reserved", [64]]]),

					["name", "string"],
					["descriptionLink", "string"],

					...(accountVersion === ACCOUNT_VERSION_V1 ? [] : [["vetoVoteWeight", "u64"]]),
				],
			},
		],
		[
		...
		[
			ProposalTransaction,
			{
				...
				fields: [
					["accountType", "u8"],
					["proposal", "pubkey"],
					accountVersion >= ACCOUNT_VERSION_V2 ? ["optionIndex", "u8"] : undefined,
					["instructionIndex", "u16"],
					["holdUpTime", "u32"],
					accountVersion >= ACCOUNT_VERSION_V2 ? ["instructions", [InstructionData]] : ["instruction", InstructionData],
...
export function getGovernanceSchemaForAccount(accountType: GovernanceAccountType) {
...
export const GovernanceAccountParser = (classType: GovernanceAccountClass) =>
	BorshAccountParser(classType, (accountType: GovernanceAccountType) => getGovernanceSchemaForAccount(accountType));
...
export async function getVersionedTransactionProposalData(
	connection: Connection,
	vtxPk: PublicKey,
): Promise<ProposalVersionedTransaction | null> {
	return connection.getAccountInfo(vtxPk).then((account) => {
		if (!account) return null;
		return GovernanceAccountParser(ProposalVersionedTransaction)(vtxPk, account).account;
	});
}
 ...
 */
export async function getAccountMetasAndLookupTableAccountsForExecuteTransaction(
	connection: Connection,
	...
	return await accountsForTransactionExecute({
		connection: connection,
