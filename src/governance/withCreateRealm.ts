...
export async function withCreateRealm(
	...
	communityTokenProgram: PublicKey = TOKEN_PROGRAM_ID,
	councilTokenProgram: PublicKey = TOKEN_PROGRAM_ID,
) {
	...
	const communityTokenHoldingAddress = await getTokenHoldingAddress(programId, realmAddress, communityMint);
	...
	if (councilMint) {
		const councilTokenHoldingAddress = await getTokenHoldingAddress(programId, realmAddress, councilMint);
	...
	if (
		...
		(communityTokenConfig?.voterWeightAddin || communityTokenConfig?.maxVoterWeightAddin)
	) {
		keys.push(realmConfigMeta);
	}
