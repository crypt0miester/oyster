import { Keypair, PublicKey, SYSVAR_CLOCK_PUBKEY, SYSVAR_RENT_PUBKEY, TransactionInstruction } from "@solana/web3.js";
import { getGovernanceInstructionSchema } from "./serialisation";
import { serialize } from "borsh";
import { CreateProposalArgs } from "./instructions";
import { getProposalDepositAddress, getRealmConfigAddress, GOVERNANCE_PROGRAM_SEED, type VoteType } from "./accounts";
import { PROGRAM_VERSION_V1, PROGRAM_VERSION_V2, PROGRAM_VERSION_V3 } from "../registry/constants";
...
export const withCreateProposal = async (
	...
	const [proposalAddress] = await PublicKey.findProgramAddress(
		[Buffer.from(GOVERNANCE_PROGRAM_SEED), governance.toBuffer(), governingTokenMint.toBuffer(), proposalSeedBuffer],
		programId,
	);
	...
	await withRealmConfigPluginAccounts(keys, programId, realm, voterWeightRecord);

	if (programVersion >= PROGRAM_VERSION_V3) {
		const proposalDepositAddress = await getProposalDepositAddress(programId, proposalAddress, payer);
		keys.push({
