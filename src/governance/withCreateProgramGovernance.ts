...
export const withCreateProgramGovernance = async (
	...
	const [governanceAddress] = await PublicKey.findProgramAddress(
		[Buffer.from("program-governance"), realm.toBuffer(), governedProgram.toBuffer()],
		programId,
	);

	const [programDataAddress] = await PublicKey.findProgramAddress([governedProgram.toBuffer()], BPF_UPGRADE_LOADER_ID);
