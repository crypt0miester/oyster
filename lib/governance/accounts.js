...
class ProposalVersionedTransaction {
    ...
    convertCompiledInstructionToInstructionData(compiledInstruction) {
        const programId = this.message.accountKeys[compiledInstruction.programIdIndex];
        const accounts = compiledInstruction.accountIndexes.map(index => {
    ...
    getAllInstructions() {
        return this.message.instructions.map(instruction => this.convertCompiledInstructionToInstructionData(instruction));
    }
    getSingleInstruction() {
